...
class IndexStore:
    ...
    def get_chunks_by_rule_number(self, rule_number: str) -> List[Chunk]:
        """Return all chunks matching the given rule_number, sorted by chunk_order."""
        matching = [
            c for c in self.chunks
            if c.rule_number is not None and c.rule_number == rule_number
        ]
        matching.sort(key=lambda c: c.chunk_order)
        return matching
