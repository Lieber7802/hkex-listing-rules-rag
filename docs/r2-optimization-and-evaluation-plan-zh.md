...
4. 以 B3、A1、A2、A3 四组正式比较：A1 是当前生产 Agent，A2/A3 是单因子消融，结果可直接解释每项 Agent 能力的贡献。
...
4. 不复制旧工作流来伪造消融；A2/A3 必须只通过显式开关分别关闭 coverage-driven second retrieval 与工具执行。
...
### 4.4 工作包 O4：可复现的正式配置与单因子消融

正式 A1 必须与生产工作流使用相同策略：`coverage_aware` evidence selection、`regulatory_grounded` tool evidence routing 和 `coverage_grounded` answer contract。A2 与 A3 继承这三项策略，因此只改变一个因素：

| 配置项 | A1 | A2 | A3 |
| --- | --- | --- | --- |
| `enable_coverage_retry` / `max_retrieval_rounds` | `True` / 2 | `False` / 1 | `True` / 2 |
| `enable_tools` | `True` | `True` | `False` |
| 三项生产策略 | 保持 | 保持 | 保持 |

保留 `A1-legacy` 与 `A1-new` 仅用于历史开发实验的可复现性，不属于本轮正式结果。B3 不读取 Agent 策略。这样四组实验可在同一提交、同一索引、同一模型和同一评测框架内运行，避免把过期策略误当成当前系统。
...
2. 在本文件或评测协议中登记 R2 的系统假设、四组配置、主指标、通过门槛和停止条件。
...
3. 一次性执行 B3、A1、A2、A3，四组使用完全相同的 `v1.1` 案例、索引、并发设置和运行环境。
...
| A1 | 当前生产 Agentic 工作流 | 判断完整 Agent 是否优于传统 RAG |
| A2 | A1，但关闭 coverage-driven second retrieval | 检验二次检索是否改善证据覆盖 |
| A3 | A1，但关闭确定性工具执行 | 检验工具是否提升工具型合规任务正确性 |

A2 与 A3 不是历史版本，而是当前 A1 的单因子消融。论文以 A1 对 B3 作为主比较，并将 A1 对 A2、A1 对 A3 报告为机制归因结果；不能把任一消融写成生产系统。
...
2. **主要确认性比较：** A1 对 B3 的 GAC。只有其差异的 95% CI 下界大于 0，才在论文中宣称“Agentic RAG 在本基准上提高有依据的答案完整性”。
3. **机制归因比较：** A1 对 A2 和 A1 对 A3 的 GAC、工具任务正确率、覆盖与检索轮次用于解释 coverage loop 与工具的贡献；它们不替代主比较。
4. **保护门槛：** A1 的运行失败率不高于 5%，工具任务正确率不低于 80%，P95 延迟不高于 24 秒，引用精确率相对 B3 的下降不超过 2 个百分点。
5. 如 A1 未优于 B3，应如实写为“当前 Agent 的机制性指标可能改善，但尚未证明总体优于传统 RAG”，而不是选择性宣传。
...
2. Planner 与 judge 使用温度 0；若所用服务不提供真实确定性/seed 保证，按现行评测协议对四组都进行三次完整重复，并报告均值与方差。
...
3. **R2 确认：** 在新的 `v1.1` 上，用 B3、A1、A2、A3 比较有依据的答案完整性、工具正确率和延迟，并用 A2/A3 解释关键机制。
...
- [ ] R2 配置可在同一代码提交中复现 B3、A1、A2、A3，且 A2/A3 仅改变登记的单一开关。
